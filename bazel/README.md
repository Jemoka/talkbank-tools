...
React SPA, mdbook builds the book, `cargo tauri build` bundles the desktop
app), Bazel **shells out** to it via
...
| 3. App bundlers | `tauri/` | shared Tauri CLI install rule (`cargo_tauri`). | Batchalign desktop bundler |
...
target; the routine flow is automatic via `tools/bazel`).
...
- Scripts assume their host tool (`uv`, `maturin`, `mdbook`, `npm`, or
  `cargo`) is on `$PATH`. CI installs them
