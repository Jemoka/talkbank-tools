...
2. **TDD gate:** per-construct corpus tests under `grammar/test/resources/corpus/word/` — `*_in_word_lint.txt`, `lengthening*.txt`, `stacked_ca_markers.txt` (the consolidated `word_segment_purity.txt` was retired in commit `fdceeac2`; current layout splits the same coverage across per-construct files generated by `just spec gen-tree-sitter-tests && just spec gen-rust-tests && just spec gen-error-docs`)
