...
flowchart TD
    ...
    spec["resources/spec/docs/\nSpec authoring guides"]
    errors["book/src/operations/errors/\nGenerated error reference"]
...
| Spec authoring guides | `resources/spec/docs/` | Error spec format, curation workflow |
| Generated error docs | `book/src/operations/errors/` | Auto-generated by the current spec/error-doc pipeline (`just spec gen-tree-sitter-tests && just spec gen-rust-tests && just spec gen-error-docs` today) |
...
3. **Satellites stay only when the audience is editing files in that directory.**
   ...
   reads the book.
4. **Generated docs are build artifacts.** Never hand-edit `book/src/operations/errors/`. Regenerate
   via the current error-doc generation pipeline (`just spec gen-tree-sitter-tests && just spec gen-rust-tests && just spec gen-error-docs` today).
