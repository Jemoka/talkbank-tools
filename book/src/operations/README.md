...
| Path | Role |
|---|---|
| `errors/` | Auto-generated validation-error catalog (one page per error code, regenerated by `just spec gen-tree-sitter-tests && just spec gen-rust-tests && just spec gen-error-docs` from `resources/spec/errors/`). Pages are not in the rendered TOC — open individual `E###.md` files in source. |
