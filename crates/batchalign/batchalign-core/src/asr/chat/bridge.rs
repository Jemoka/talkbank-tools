...
pub enum TranscriptBuildError {
    /// A transcript or utterance supplied an empty language code.
    #[error("invalid language code {language:?}: {source}")]
    InvalidLanguage {
        language: String,
        #[source]
...
/// braces.
///
/// [try_lang]: asr::ChatWordText::try_from_lang
pub fn transcript_from_asr_utterances(
    ...
    let primary_language = langs.first().map(String::as_str).unwrap_or("eng");
    let primary_lang_code = LanguageCode::new(primary_language).map_err(|source| {
        TranscriptBuildError::InvalidLanguage {
    ...
    for (utt_idx, utterance) in utterances.iter().enumerate() {
        ...
        let utterance_lang = utterance
            ...
            .as_deref()
            .map(|language| {
                LanguageCode::new(language).map_err(|source| {
                    TranscriptBuildError::InvalidLanguage {
...
fn build_asr_participants(
    ...
    let mut seen_speakers: Vec<asr::SpeakerIndex> = Vec::new();
...
fn resolve_speaker_id(speaker: asr::SpeakerIndex, participant_ids: &[String]) -> String {
...
fn validate_asr_word(
    ...
    let text = match asr::ChatWordText::try_from_lang(word.text.as_str(), utterance_lang) {
        Ok(text) => text,
...
) -> Result<asr::ChatWordText, TranscriptBuildError> {
    match asr::ChatWordText::try_from(word.text.as_str()) {
        Ok(structural) => {
            tracing::warn!(
                utt_idx,
                word_idx,
