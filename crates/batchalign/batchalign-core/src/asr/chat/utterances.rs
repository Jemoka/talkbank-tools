...
fn apply_utterance_language_override(
    ...
    {
        utterance.main.content.language_code = Some(
            LanguageCode::new(utterance_lang)
                .map_err(|error| format!("invalid utterance language: {error}"))?,
        );
    }
    Ok(())
}
...
fn build_text_utterance(
    ...
    let parsed = talkbank_transform::parse::parse_strict(parser, &mini_chat).map_err(|error| {
...
fn build_word_utterance(
    ...
    while index < words.len() {
        ...
        if word.kind == asr::WordKind::Retrace {
...
fn push_retrace_run(
    ...
    while end_index < words.len() && words[end_index].kind == asr::WordKind::Retrace {
        end_index += 1;
    }
