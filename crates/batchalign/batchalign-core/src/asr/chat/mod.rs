...
use talkbank_model::model::{
    ChatFile, Header, IDHeader, LanguageCode, Line, Participant, ParticipantEntry, ParticipantName,
...
pub fn build_chat(desc: &TranscriptDescription) -> Result<ChatFile, String> {
    ...
    let lang_code = LanguageCode::new(context.primary_lang())
    ...
    let participants = desc
        ...
        .map(|participant| {
            ...
            let id = IDHeader::new(lang_code.clone(), code.clone(), role).with_corpus(corpus);
